...
def start_button_updater(chat_id: int, message, markup_fn, video_id: str, total_sec: int):
    """Updates only the progress-button row, leaving the song caption untouched."""
    cancel_task(chat_id)
    if total_sec <= 0:
        return
    ...
    async def _runner():
        ...
        try:
            while True:
                ...
                try:
                    await message.edit_reply_markup(reply_markup=markup_fn(el, total_sec))
                    failures = 0
                    ...
                    if "MESSAGE_NOT_MODIFIED" in str(e):
                        ...
                        failures += 1
                        LOGGER.warning(f"Progress button update fail ({failures}): {e}")
