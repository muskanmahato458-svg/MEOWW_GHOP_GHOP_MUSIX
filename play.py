...
# REPO button — click karne par yeh popup dikhta hai (bina emoji, baaki
# buttons jaisi hi smallcaps style mein)
REPO_ALERT_TEXT = (
    ...
    f"{smallcaps_title('contact')} @nexor_blaze {smallcaps_title('on telegram for paid setup')}!"
)
...
def _progress_keyboard(elapsed_sec: float = 0, total_sec: int = 0):
    """Live seek/progress strip as a real inline button — bar ke saath
    elapsed/total time bhi text mein dikhta hai (seek karne par turant
    update hota hai)."""
    length = 10
    if total_sec > 0:
        ...
        label = f"{format_duration(int(elapsed_sec))} {bar} {format_duration(int(total_sec))}"
    else:
        bar = "▰" * (length - 1) + "▱"
        label = bar
    return [_btn(label, callback_data="noop", style="primary")]


def _controls_keyboard(elapsed_sec: float = 0, total_sec: int = 0):
    return InlineKeyboardMarkup(
        [
            _progress_keyboard(elapsed_sec, total_sec),
            [
            ...
            [
                ...
                _btn(smallcaps_title("repo"), callback_data="m_repo", style="primary"),
...
def _owner_text() -> str:
    ...
    body = expandable_blockquote(
        f"{smallcaps_title('this bot is proudly built and maintained by my owner')} — "
...
async def start_cmd(client, message: Message):
    ...
    # Private ho ya group, hamesha greeting-style welcome hi jaata hai —
    # uptime/"is online" wala alag start message code se hata diya gaya hai,
    # ab kahin bhi nahi aayega.
...
async def play_command(client, message: Message):
    ...
    # Assistant-membership check aur YouTube search dono ek-doosre pe depend
    # nahi karte — pehle sequentially chalte the (dono ka wait time jud jaata
    # tha), ab parallel chalte hain taaki /play jaldi respond kare.
    assistant_task = asyncio.create_task(ensure_assistant_in_chat(chat_id))
    search_task = asyncio.create_task(search_track(query))

    # Sirf ek single emoji dikhana hai — koi "Searching..." text nahi, koi
    # progress-bar button nahi (jaisa maanga gaya tha). Turant bhej dete hain,
    # user ko instant feedback milta hai.
    status = await message.reply_text(random_processing_text())

    joined, reason = await assistant_task
    if not joined:
        search_task.cancel()
        if reason == "flood_wait":
            return await status.edit_text(ASSISTANT_FLOOD_TEXT, reply_markup=None)
        return await status.edit_text(ASSISTANT_NOT_JOINED_TEXT, reply_markup=None)

    track = await search_task
    if not track:
        return await status.edit_text(
            f"❌ {smallcaps_title('kuch nahi mila, doosra naam try karo')}.", reply_markup=None
        )
    ...
    if q.is_playing(chat_id):
        ...
        me = await bot.get_me()
        await message.reply_text(
            _queue_caption(track, position),
            reply_markup=_add_me_keyboard(me.username),
            disable_web_page_preview=True,
        )
        return
...
def _processing_keyboard():
    """/play chalte hi progress-bar jaisa dikhne wala button (khud koi
    action nahi karta — sirf visual "processing" indicator)."""
    return InlineKeyboardMarkup([[_btn("▰▰▰▰▰▰▰▰▰▰▰▰▱", callback_data="noop", style="primary")]])


@bot.on_callback_query(filters.regex("^noop$"))
async def _noop_cb(client, cq: CallbackQuery):
    await cq.answer()


def _add_me_keyboard(bot_username: str):
    """Sirf 'Add me' button — queue message ke neeche."""
    return InlineKeyboardMarkup(
        [
            [
                _btn(
...
# ---------------------------------------------------------------------------
# Song-details box — screenshot jaisa "» 『 Title 』 / ╰┈➤ ..." style, jitni
# lines mile utni dikha deta hai (channel/uploader) — koi bhi field missing
# ho to woh line hi skip ho jaati hai, kabhi khaali line nahi dikhti.
# ---------------------------------------------------------------------------
def _song_details_box(track: dict) -> str:
    lines = [f"» 『 {esc(track['title'])} 』"]
    channel = track.get("channel")
    if channel:
        lines.append(f"╰┈➤ {smallcaps_title(str(channel))}")
    views = track.get("views")
    if views:
        lines.append(f"╰┈➤ {smallcaps_title('views')} : {esc(views)}")
    return expandable_blockquote("\n".join(lines))


def _queue_caption(track: dict, position: int) -> str:
    header = blockquote(f"❖ {smallcaps_title('queued to play')} #{position}..!! ✦")
    song_box = _song_details_box(track)
    meta = expandable_blockquote(
        f"⌾ {smallcaps_title('duration')} : {esc(track['duration'])}\n"
        f"⌾ {smallcaps_title('requested by')} : {track.get('requested_by', 'Unknown')}"
    )
    return f"{header}\n\n{song_box}\n\n{meta}"


def _now_playing_caption(track: dict) -> str:
    # 3 alag quote-box: (1) header, (2) scrollable song-details,
    # (3) duration + requested-by — jaisa maanga gaya tha.
...
async def _send_now_playing(chat_id: int, track: dict, message: Message = None, edit_message: Message = None):
    ...
    # 🎚️ Live progress bar ab caption ke andar nahi, inline button ke roop mein
    # update hota hai. Isse song details static quote-box mein rehti hain.
    if sent is not None:
        progress.start(chat_id, track["id"])
        progress.start_button_updater(
            chat_id, sent,
            _controls_keyboard,
            track["id"], total_sec,
        )
...
async def _handle_seek(chat_id: int, cq: CallbackQuery, delta: int):
    ...
    # Progress bar button turant naye position par dikhe — 3s wale background
    # updater ka wait nahi karna, warna seek ke baad time purana dikhta reh jaata.
    try:
        await cq.message.edit_reply_markup(reply_markup=_controls_keyboard(new_elapsed, total_sec))
    except Exception as e:
        if "MESSAGE_NOT_MODIFIED" not in str(e):
            LOGGER.warning(f"Seek keyboard refresh fail: {e}")
            ...
            if not next_track:
                ...
                try:
                    ...
                    try:
                        ...
                        LOGGER.warning(f"Inline skip error: {e2}")
                        return await cq.message.reply_text(
                            f"❌ {smallcaps_title('skip nahi ho paya, dobara try karo')}."
                        )
                q.set_now_playing(chat_id, next_track)
