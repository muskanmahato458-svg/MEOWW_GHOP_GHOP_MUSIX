...
def _progress_keyboard(elapsed_sec: float = 0, total_sec: int = 0):
    """Live seek/progress strip as a real inline button.
    The bar is updated in the keyboard instead of editing the caption.
    """
    # Compact live progress button — Telegram ke button ke andar sirf
    # requested visual bar rahega, taaki button message/text se bahar na nikle.
    length = 13
    if total_sec > 0:
...
async def start_cmd(client, message: Message):
    ...
    if message.chat.type != "private":
        # Group mein "<bot> is online" + live uptime dikhana hai — private
        # wali personal greeting yahan nahi jaani chahiye.
...
async def play_command(client, message: Message):
    ...
    # progress-bar button nahi (jaisa maanga gaya tha).
