...
def _progress_keyboard(elapsed_sec: float = 0, total_sec: int = 0):
    ...
    if total_sec > 0:
        ...
        left = progress.format_duration(int(elapsed_sec))
        right = progress.format_duration(int(total_sec))
        label = f"{left}  {bar}  {right}"
    else:
        label = "00:00  ──────────────────  --:--"
    ...
    return InlineKeyboardMarkup(
        [
            ...
            [
                ...
                _btn(smallcaps_title("repo"), callback_data="m_repo"),
...
def _now_playing_caption(track: dict) -> str:
    ...
    body = expandable_blockquote(f"» 『 {esc(track['title'])} 』")
    meta = expandable_blockquote(
        f"⌾ {smallcaps_title('duration')} : {esc(track['duration'])}\n"
