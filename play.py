...
async def start_cmd(client, message: Message):
    ...
    if message.chat.type != "private":
        # Group mein bhi wahi greeting/welcome message use hota hai — old
        # "is online / uptime" start message hata diya gaya hai.
...
async def play_command(client, message: Message):
    ...
    status = await message.reply_text(
        f"🔎 {smallcaps_title('searching')}...", reply_markup=_processing_keyboard()
    )
...
def _queue_caption(track: dict, position: int) -> str:
    ...
    body = expandable_blockquote(f"» 『 #{position} • {esc(track['title'])} 』")
